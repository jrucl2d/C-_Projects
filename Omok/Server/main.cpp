...
void ServerThread(Client* client) {
	...
	while (true) {
		...
		if ((size = recv(client->getClientSocket(), received, 256, NULL)) > 0) { // 클라이언트로부터 메시지를 전달 받음
...
int main() {
	...
	while (true) {
		...
		if (clientSocket = accept(serverSocket, (SOCKADDR*)&serverAddress, &addressLength)) {
			...
			CreateThread(NULL, NULL, (LPTHREAD_START_ROUTINE)ServerThread, (LPVOID)client, NULL, NULL); // 사용자 한 명 한 명에 대해서 서버 스레드 생성
